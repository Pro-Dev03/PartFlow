	...
	base := fmt.Sprintf(`
		...
		FROM (
			...
			SELECT
				...
				CASE WHEN im.item_id IS NOT NULL AND im.item_id <> '' THEN 'inventory_item' ELSE 'product' END AS entity_type,
				...
				CASE WHEN COALESCE(im.is_reversed, 0) <> 0 THEN 'reversed' ELSE 'completed' END AS status,
