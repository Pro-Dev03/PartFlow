...
func NewAggregationHandler(db *sqlx.DB) *AggregationHandler {
	if db != nil && dbutil.IsSQLite(db) {
		ensureSQLiteAggregationReturnViews(db.DB)
	}
...
func (h *AggregationHandler) sqliteSaleTaxExpression(ctx context.Context) string {
	...
	for rows.Next() {
		...
		if strings.EqualFold(name, "tax_amount") {
			return "COALESCE(s.tax_amount, 0)"
		}
	}
	if err := rows.Err(); err != nil {
		return "0"
...
func (h *AggregationHandler) sqliteProductCostExpression(ctx context.Context) string {
	rows, err := h.db.QueryxContext(ctx, "PRAGMA table_info(products)")
	if err != nil {
	...
	for rows.Next() {
		...
		switch {
		case strings.EqualFold(name, "cost_price"):
			hasCostPrice = true
		case strings.EqualFold(name, "purchase_price"):
	...
	if hasPurchasePrice {
		return "COALESCE(ii.purchase_cost, p.purchase_price, 0)"
	}
	return "COALESCE(ii.purchase_cost, p.cost_price, p.purchase_price, 0)"
}
...
func buildDailySalesSummaryQueryForDB(db *sqlx.DB) string {
	...
	return `
		SELECT
			...
			COALESCE(s.card_sales, 0) AS card_sales,
			COALESCE(s.debt_sales, 0) AS debt_sales,
			COALESCE(s.updated_at::text, NOW()::text) AS updated_at
...
func (h *AggregationHandler) refreshSQLiteSummaries(ctx context.Context, startDate, endDate time.Time) error {
	...
	if _, err := h.db.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS inventory_movements (
		id TEXT PRIMARY KEY,
...
// sqliteReturnProfitAvailable reports whether the optional return-cost fields
// exist before running return-aware summary SQL. This keeps migrations and
// older local databases backwards compatible.
func (h *AggregationHandler) sqliteReturnProfitAvailable(ctx context.Context) bool {
	requiredColumns := map[string][]string{
		"returns":      {"total_refund_amount", "status", "return_date"},
		"return_items": {"quantity_returned", "original_cost", "sale_item_id", "product_id"},
	}
...
func (h *AggregationHandler) refreshPostgresSummaries(ctx context.Context, startDate, endDate time.Time) error {
	...
	for ; !day.After(endDay); day = day.AddDate(0, 0, 1) {
		...
		queries := []string{
			...
			`INSERT INTO daily_debt_summary (summary_date,date,total_debt,new_debt,payments_received,overdue_debt,overdue_count,paid_debt,updated_at)
...
func (h *AggregationHandler) refreshSQLiteDay(ctx context.Context, day time.Time) error {
	date := day.Format("2006-01-02")
	productCostExpr := h.sqliteProductCostExpression(ctx)
	productPriceRef := "p.cost_price"
	if !strings.Contains(productCostExpr, "p.cost_price") {
		productPriceRef = "p.purchase_price"
	}
	queries := []struct {
		query string
		args  []any
		...
		{`INSERT OR REPLACE INTO daily_sales_summary (date, total_sales, total_revenue, total_profit, total_customers, average_order_value, total_items_sold, cash_sales, card_sales, debt_sales, updated_at)
			SELECT ?, COUNT(s.id), COALESCE(SUM(s.total_amount - COALESCE(s.tax_amount, 0)), 0), COALESCE(SUM(s.total_amount - COALESCE(s.tax_amount, 0) - COALESCE(cost.total_cost, 0)), 0),
			COUNT(DISTINCT s.customer_id), COALESCE(SUM(s.total_amount - COALESCE(s.tax_amount, 0)) / NULLIF(COUNT(s.id), 0), 0),
			COALESCE(SUM(cost.total_items), 0),
			COALESCE(SUM(CASE WHEN lower(COALESCE(s.payment_method, '')) IN ('cash', 'cash_payment') THEN s.total_amount - COALESCE(s.tax_amount, 0) ELSE 0 END), 0),
			COALESCE(SUM(CASE WHEN lower(COALESCE(s.payment_method, '')) IN ('card', 'credit_card') THEN s.total_amount - COALESCE(s.tax_amount, 0) ELSE 0 END), 0),
			COALESCE(SUM(CASE WHEN lower(COALESCE(s.payment_method, '')) IN ('debt', 'credit', 'on_account') THEN s.total_amount - COALESCE(s.tax_amount, 0) ELSE 0 END), 0), CURRENT_TIMESTAMP
			FROM sales s LEFT JOIN (SELECT si.sale_id, SUM(si.quantity) AS total_items, SUM(si.quantity * COALESCE(ii.purchase_cost, p.cost_price, 0)) AS total_cost FROM sale_items si LEFT JOIN inventory_items ii ON ii.id = si.inventory_item_id LEFT JOIN products p ON p.id = si.product_id GROUP BY si.sale_id) cost ON cost.sale_id = s.id
		...
		{`INSERT OR REPLACE INTO daily_inventory_summary (date, total_items, total_value, low_stock_count, out_of_stock_count, new_items_added, items_sold, items_returned, items_damaged, updated_at)
			...
			(SELECT COALESCE(SUM(ri.quantity_returned), 0) FROM accounting_return_items ri JOIN accounting_returns r ON r.id = ri.return_id WHERE date(COALESCE(r.return_date, r.created_at)) = ? AND lower(COALESCE(r.status, 'completed')) = 'completed'),
		...
		{`INSERT OR REPLACE INTO daily_profit_summary (date, gross_profit, net_profit, total_revenue, total_cost, profit_margin, updated_at)
			WITH sale_costs AS (SELECT s.id, s.total_amount, COALESCE(s.tax_amount, 0) AS tax_amount, COALESCE(SUM(si.quantity * COALESCE(ii.purchase_cost, p.cost_price, 0)), 0) AS total_cost FROM sales s LEFT JOIN sale_items si ON si.sale_id = s.id LEFT JOIN inventory_items ii ON ii.id = si.inventory_item_id LEFT JOIN products p ON p.id = si.product_id WHERE date(COALESCE(s.sale_date, s.created_at)) = ? AND lower(COALESCE(s.status, 'completed')) = 'completed' GROUP BY s.id), totals AS (SELECT COALESCE(SUM(total_amount - tax_amount), 0) AS revenue, COALESCE(SUM(total_cost), 0) AS cost FROM sale_costs), expenses_total AS (SELECT COALESCE(SUM(amount), 0) AS amount FROM expenses WHERE date(expense_date) = ? AND lower(COALESCE(status, 'approved')) IN ('approved', 'paid', 'completed', 'archived')), returns_total AS (SELECT COALESCE(SUM(r.total_refund_amount), 0) AS refunded, COALESCE(SUM(ri.quantity_returned * COALESCE(ri.original_cost, si.unit_cost, p.cost_price, 0)), 0) AS returned_cost FROM accounting_returns r JOIN accounting_return_items ri ON ri.return_id = r.id LEFT JOIN sale_items si ON si.id = ri.sale_item_id LEFT JOIN products p ON p.id = si.product_id WHERE date(r.return_date) = ? AND upper(COALESCE(r.status, '')) = 'COMPLETED')
...
func (h *AggregationHandler) refreshSQLiteMonth(ctx context.Context, month time.Time) error {
	...
	queries := []struct {
		...
		{`INSERT OR REPLACE INTO monthly_sales_summary (year, month, total_sales, total_revenue, total_profit, total_customers, average_order_value, total_items_sold, cash_sales, card_sales, debt_sales, updated_at)
			SELECT ?, ?, COUNT(s.id), COALESCE(SUM(s.total_amount - COALESCE(s.tax_amount, 0)), 0), COALESCE(SUM(s.total_amount - COALESCE(s.tax_amount, 0) - COALESCE(cost.total_cost, 0)), 0), COUNT(DISTINCT s.customer_id), COALESCE(SUM(s.total_amount - COALESCE(s.tax_amount, 0)) / NULLIF(COUNT(s.id), 0), 0), COALESCE(SUM(cost.total_items), 0),
			COALESCE(SUM(CASE WHEN lower(COALESCE(s.payment_method, '')) IN ('cash', 'cash_payment') THEN s.total_amount - COALESCE(s.tax_amount, 0) ELSE 0 END), 0), COALESCE(SUM(CASE WHEN lower(COALESCE(s.payment_method, '')) IN ('card', 'credit_card') THEN s.total_amount - COALESCE(s.tax_amount, 0) ELSE 0 END), 0), COALESCE(SUM(CASE WHEN lower(COALESCE(s.payment_method, '')) IN ('debt', 'credit', 'on_account') THEN s.total_amount - COALESCE(s.tax_amount, 0) ELSE 0 END), 0), CURRENT_TIMESTAMP
			FROM sales s LEFT JOIN (SELECT si.sale_id, SUM(si.quantity) AS total_items, SUM(si.quantity * COALESCE(ii.purchase_cost, p.cost_price, 0)) AS total_cost FROM sale_items si LEFT JOIN inventory_items ii ON ii.id = si.inventory_item_id LEFT JOIN products p ON p.id = si.product_id GROUP BY si.sale_id) cost ON cost.sale_id = s.id
		...
		{`INSERT OR REPLACE INTO monthly_profit_summary (year, month, gross_profit, net_profit, total_revenue, total_cost, profit_margin, updated_at)
			WITH sale_costs AS (SELECT s.id, s.total_amount - COALESCE(s.tax_amount, 0) AS total_amount, COALESCE(SUM(si.quantity * COALESCE(ii.purchase_cost, p.cost_price, 0)), 0) AS total_cost FROM sales s LEFT JOIN sale_items si ON si.sale_id = s.id LEFT JOIN inventory_items ii ON ii.id = si.inventory_item_id LEFT JOIN products p ON p.id = si.product_id WHERE date(COALESCE(s.sale_date, s.created_at)) >= ? AND date(COALESCE(s.sale_date, s.created_at)) < ? AND lower(COALESCE(s.status, 'completed')) = 'completed' GROUP BY s.id), totals AS (SELECT COALESCE(SUM(total_amount), 0) AS revenue, COALESCE(SUM(total_cost), 0) AS cost FROM sale_costs), expenses_total AS (SELECT COALESCE(SUM(amount), 0) AS amount FROM expenses WHERE date(expense_date) >= ? AND date(expense_date) < ? AND lower(COALESCE(status, 'approved')) IN ('approved', 'paid', 'completed', 'archived')), returns_total AS (SELECT COALESCE(SUM(r.total_refund_amount), 0) AS refunded, COALESCE(SUM(ri.quantity_returned * COALESCE(ri.original_cost, si.unit_cost, p.cost_price, 0)), 0) AS returned_cost FROM accounting_returns r JOIN accounting_return_items ri ON ri.return_id = r.id LEFT JOIN sale_items si ON si.id = ri.sale_item_id LEFT JOIN products p ON p.id = ri.product_id WHERE date(r.return_date) >= ? AND date(r.return_date) < ? AND upper(COALESCE(r.status, '')) = 'COMPLETED')
			SELECT ?, ?, totals.revenue - totals.cost - returns_total.refunded + returns_total.returned_cost, totals.revenue - totals.cost - returns_total.refunded + returns_total.returned_cost - expenses_total.amount, totals.revenue - returns_total.refunded, totals.cost - returns_total.returned_cost, CASE WHEN totals.revenue - returns_total.refunded = 0 THEN 0 ELSE ((totals.revenue - totals.cost - returns_total.refunded + returns_total.returned_cost - expenses_total.amount) / (totals.revenue - returns_total.refunded)) * 100 END, CURRENT_TIMESTAMP FROM totals, expenses_total, returns_total
	...
	if h.sqliteHasColumns(ctx, "returns", "total_refund_amount", "return_date", "status") && h.sqliteHasColumns(ctx, "return_items", "quantity_returned", "original_cost") {
		queries = append(queries, struct {
			...
			SET total_revenue = MAX(0, total_revenue - COALESCE((SELECT SUM(total_refund_amount) FROM accounting_returns WHERE upper(COALESCE(status, '')) = 'COMPLETED' AND date(COALESCE(return_date, created_at)) >= ? AND date(COALESCE(return_date, created_at)) < ?), 0)),
				total_profit = total_profit - COALESCE((SELECT SUM(total_refund_amount) FROM accounting_returns WHERE upper(COALESCE(status, '')) = 'COMPLETED' AND date(COALESCE(return_date, created_at)) >= ? AND date(COALESCE(return_date, created_at)) < ?), 0) + COALESCE((SELECT SUM(COALESCE(ri.quantity_returned, 0) * COALESCE(ri.original_cost, si.unit_cost, p.cost_price, 0)) FROM accounting_returns r JOIN accounting_return_items ri ON ri.return_id = r.id LEFT JOIN sale_items si ON si.id = ri.sale_item_id LEFT JOIN products p ON p.id = ri.product_id WHERE upper(COALESCE(r.status, '')) = 'COMPLETED' AND date(COALESCE(r.return_date, r.created_at)) >= ? AND date(COALESCE(r.return_date, r.created_at)) < ?), 0)
	...
	if !h.sqliteReturnProfitAvailable(ctx) {
		queries[3] = struct {
			...
			WITH sale_costs AS (SELECT s.id, s.total_amount - COALESCE(s.tax_amount, 0) AS total_amount, COALESCE(SUM(si.quantity * COALESCE(ii.purchase_cost, p.cost_price, 0)), 0) AS total_cost FROM sales s LEFT JOIN sale_items si ON si.sale_id = s.id LEFT JOIN inventory_items ii ON ii.id = si.inventory_item_id LEFT JOIN products p ON p.id = si.product_id WHERE date(COALESCE(s.sale_date, s.created_at)) >= ? AND date(COALESCE(s.sale_date, s.created_at)) < ? AND lower(COALESCE(s.status, 'completed')) = 'completed' GROUP BY s.id), totals AS (SELECT COALESCE(SUM(total_amount), 0) AS revenue, COALESCE(SUM(total_cost), 0) AS cost FROM sale_costs), expenses_total AS (SELECT COALESCE(SUM(amount), 0) AS amount FROM expenses WHERE date(expense_date) >= ? AND date(expense_date) < ? AND lower(COALESCE(status, 'approved')) IN ('approved', 'paid', 'completed', 'archived'))
			SELECT ?, ?, totals.revenue - totals.cost, totals.revenue - totals.cost - expenses_total.amount, totals.revenue, totals.cost, CASE WHEN totals.revenue = 0 THEN 0 ELSE ((totals.revenue - totals.cost - expenses_total.amount) / totals.revenue) * 100 END, CURRENT_TIMESTAMP FROM totals, expenses_total
