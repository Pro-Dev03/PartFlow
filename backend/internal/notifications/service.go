...
func (s *Service) ListNotifications(ctx context.Context, userID uuid.UUID, req NotificationListRequest) ([]Notification, int, error) {
	if req.Page <= 0 {
		req.Page = 1
	}
	...
	notifications, total, err := s.repo.ListNotifications(ctx, userID, req)
	if err != nil {
		return nil, 0, err
	}
...
func (s *Service) GetNotificationSummary(ctx context.Context, userID uuid.UUID) (*NotificationSummary, error) {
	return s.repo.GetNotificationSummary(ctx, userID)
}

// CreateNotificationPreferences creates notification preferences for a user
func (s *Service) CreateNotificationPreferences(ctx context.Context, userID uuid.UUID) (*NotificationPreferences, error) {
...
func (s *Service) GetNotificationPreferences(ctx context.Context, userID uuid.UUID) (*NotificationPreferences, error) {
	...
	if err != nil {
		...
		if err == ErrPreferencesNotFound {
			return s.CreateNotificationPreferences(ctx, userID)
		}
		return nil, err
	}
...
func (s *Service) UpdateNotificationPreferences(ctx context.Context, userID uuid.UUID, preferences *NotificationPreferences) (*NotificationPreferences, error) {
	// Get existing preferences first
	existing, err := s.repo.GetNotificationPreferences(ctx, userID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) SendLowStockNotification(ctx context.Context, userID uuid.UUID, productName string, currentStock int, minStock int) error {
...
func (s *Service) SendDebtOverdueNotification(ctx context.Context, userID uuid.UUID, customerName string, amount float64, overdueDays int) error {
	req := &NotificationRequest{
		UserID:     userID,
		Type:       "debt_overdue",
...
func (s *Service) SendReturnRequestNotification(ctx context.Context, userID uuid.UUID, customerName string, productName string) error {
	req := &NotificationRequest{
		UserID:     userID,
		Type:       "return_request",
...
func (s *Service) SendExpenseApprovalNotification(ctx context.Context, userID uuid.UUID, expenseTitle string, amount float64) error {
	req := &NotificationRequest{
		UserID:     userID,
