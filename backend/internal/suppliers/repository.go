...
func localSupplierQuery() string {
	return `SELECT s.id, s.code, s.name, s.email, s.phone, s.address, s.city, s.country, s.tax_id, s.payment_terms, s.credit_limit, s.current_balance, COALESCE((SELECT SUM(total_amount) FROM purchases p WHERE p.supplier_id = s.id AND p.status NOT IN ('cancelled','reversed')), 0) AS total_purchases, COALESCE((SELECT SUM(paid_amount) FROM purchases p WHERE p.supplier_id = s.id AND p.status NOT IN ('cancelled','reversed')), 0) + COALESCE((SELECT SUM(amount) FROM supplier_ledger sl WHERE sl.supplier_id = s.id AND sl.type = 'credit' AND sl.transaction_type = 'PAYMENT' AND NOT EXISTS (SELECT 1 FROM purchases p WHERE p.id = sl.reference_id)), 0) AS paid_amount, COALESCE((SELECT SUM(total_amount) FROM purchases p WHERE p.supplier_id = s.id AND p.status NOT IN ('cancelled','reversed')), 0) - COALESCE((SELECT SUM(refund_amount) FROM supplier_returns sr WHERE sr.supplier_id = s.id AND sr.status = 'COMPLETED'), 0) - (COALESCE((SELECT SUM(paid_amount) FROM purchases p WHERE p.supplier_id = s.id AND p.status NOT IN ('cancelled','reversed')), 0) + COALESCE((SELECT SUM(amount) FROM supplier_ledger sl WHERE sl.supplier_id = s.id AND sl.type = 'credit' AND sl.transaction_type = 'PAYMENT' AND NOT EXISTS (SELECT 1 FROM purchases p WHERE p.id = sl.reference_id)), 0)) AS outstanding, (SELECT MAX(p.created_at) FROM purchases p WHERE p.supplier_id = s.id AND p.status NOT IN ('cancelled','reversed')) AS last_purchase, s.notes, s.is_active, s.created_at, s.updated_at FROM suppliers s`
}
...
func (r *Repository) GetByID(ctx context.Context, id uuid.UUID) (*Supplier, error) {
	...
	query := `
		SELECT id, code, name, email, phone, address, city, country, tax_id,
			...
			COALESCE((SELECT SUM(paid_amount) FROM purchases WHERE supplier_id = suppliers.id AND status NOT IN ('cancelled', 'reversed')), 0) + COALESCE((SELECT SUM(amount) FROM supplier_ledger sl WHERE sl.supplier_id = suppliers.id AND sl.type = 'credit' AND sl.transaction_type = 'PAYMENT' AND NOT EXISTS (SELECT 1 FROM purchases p WHERE p.id = sl.reference_id)), 0) AS paid_amount,
			COALESCE((SELECT SUM(total_amount) FROM purchases WHERE supplier_id = suppliers.id AND status NOT IN ('cancelled', 'reversed')), 0) - COALESCE((SELECT SUM(refund_amount) FROM supplier_returns WHERE supplier_id = suppliers.id AND status = 'COMPLETED'), 0) - (COALESCE((SELECT SUM(paid_amount) FROM purchases WHERE supplier_id = suppliers.id AND status NOT IN ('cancelled', 'reversed')), 0) + COALESCE((SELECT SUM(amount) FROM supplier_ledger sl WHERE sl.supplier_id = suppliers.id AND sl.type = 'credit' AND sl.transaction_type = 'PAYMENT' AND NOT EXISTS (SELECT 1 FROM purchases p WHERE p.id = sl.reference_id)), 0)) AS outstanding,
