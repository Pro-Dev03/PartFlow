...
func TestAuthRejectsAccessTokenIssuedBeforePasswordChange(t *testing.T) {
	...
	if _, err := sqlDB.Exec(`CREATE TABLE users (
		...
		updated_at TEXT NOT NULL, session_version INTEGER NOT NULL DEFAULT 0
	)`); err != nil {
		t.Fatal(err)
	}
...
func TestAuthRejectsAccessTokenAfterSessionVersionChanges(t *testing.T) {
	gin.SetMode(gin.TestMode)
	previousDB := db
	t.Cleanup(func() { SetDatabase(previousDB) })
	SetDisableAuth(false)
	SetJWTSecret("session-version-test-secret")
	t.Setenv("DB_CONNECTION_MODE", "cloud")
	t.Setenv("DATABASE_URL", "postgres://example.test/partflow")
	t.Setenv("SERVER_MODE", "production")
	t.Setenv("APP_ENV", "production")
	...
	userID := uuid.New()
	now := time.Now().UTC()
	if _, err := sqlDB.Exec(`INSERT INTO users (id, is_active, subscription_status, subscription_expires_at, updated_at, session_version)
		VALUES (?, 1, 'active', ?, ?, 1)`, userID.String(), now.Add(time.Hour).Format(time.RFC3339Nano), now.Format(time.RFC3339Nano)); err != nil {
		t.Fatal(err)
	}
	SetDatabase(sqlx.NewDb(sqlDB, "sqlite"))
	...
	tokenString, err := token.SignedString([]byte("session-version-test-secret"))
	if err != nil {
		t.Fatal(err)
	}
	router := gin.New()
	...
	router.ServeHTTP(response, request)
	if response.Code != http.StatusUnauthorized || !strings.Contains(response.Body.String(), "SESSION_REVOKED") {
		t.Fatalf("old access token after logout: status=%d body=%s", response.Code, response.Body.String())
	}
}
