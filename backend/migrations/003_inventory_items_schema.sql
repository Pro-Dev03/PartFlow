...
CREATE TABLE IF NOT EXISTS inventory_items (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'PURCHASED' CHECK (status IN ('PURCHASED', 'RECEIVED', 'INSPECTION', 'AVAILABLE', 'RESERVED', 'SOLD', 'DAMAGED', 'IN_REPAIR', 'RETURNED', 'FOR_PARTS', 'ARCHIVED')),
...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO inventory_movements (item_id, product_id, movement_type, quantity, before_quantity, after_quantity, reference_type, reference_id, reason, created_by, created_at)
    VALUES (
        NEW.id,
        NEW.product_id,
        'PURCHASE',
