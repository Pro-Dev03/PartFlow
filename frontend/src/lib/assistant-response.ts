...
function smallTalk(message: string): string | undefined {
  ...
  if (/شخبارك|شو\s*حالك|كيف\s*حالك|كيف\s*أموري/.test(text)) {
    const responses = [
      'أنا بخير، تمام تمام، وكل يوم بيعرفني على وضع المتجر. شو في جديد عندك؟',
      'أنا بخير والحمد لله، وكل ما أشوف وضعك أحسن بفرح. شو بدك نراجع اليوم؟',
      'أنا بخير وماشي الحال، وكل يوم أتعلم منك شي جديد. شو اللي يخوفك اليوم؟',
      'أنا بخير وجاهز لمساعدتك. شو رأيك نطلع على المخزون أو الديون؟',
    ];
  ...
  if (/من\s*أنت|who\s*are\s*you|أنت\s*من|انت\s*من|من\s*هو\s*أنت/.test(text)) {
    const responses = [
      `أنا مساعدك الشخصي في PartFlow، واسمي ${assistantName}. بقلب عليك، بتابع المخزون، الديون، والمبيعات، وبقولك شو اللي يحتاج انتباهك أول.`,
      `اسمي ${assistantName}، وأنا مساعدك الشخصي في PartFlow. كل شي تبي تسأله عن المخزون أو الديون أو المبيعات، أنا معك.`,
      `أنا مساعدك في PartFlow، واسمي ${assistantName}، وصاحبك في المتجر. هون عشان أسهل عليك حياتك وأقولك شو لازم تعمله اليوم.`,
    ];
...
export function generateAssistantReply(message: string, context: AssistantContext): string {
  ...
  if (asksAboutCurrentStatus) {
    ...
    `أولويات اليوم واضحة: شفت شو في؟ عندك ${issues.join(' و ')}. فلا تستهين بهم، خدهم بجدية اليوم.`,
    `أولويات اليوم واضحة: ${issues.join(' و ')}. يعني لازم تبدأ بهم من الصباح عشان تنهي يومك ومرتاح.`,
    ];
