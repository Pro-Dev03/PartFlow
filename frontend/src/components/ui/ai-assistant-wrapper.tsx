  ...
  const assistantContext = useMemo(() => {
    ...
    return {
      ...
      lowStockItems: lowStockItems.slice(0, 5).map((item: any) => ({
        name: item?.name || item?.product_name || item?.productName || item?.product?.name || item?.product?.product_name || 'منتج',
        quantity: item?.quantity ?? item?.stock ?? item?.available_quantity ?? item?.current_quantity ?? 0,
        minQuantity: item?.minQuantity ?? item?.minimum_quantity ?? item?.min_stock_level ?? 0,
        category: item?.categoryName || item?.category?.name || item?.partType?.name,
      })),
      overdueDebts: overdueDebts.slice(0, 5).map((item: any) => ({
        customerName: item?.customerName || item?.customer_name || item?.customer?.name || 'عميل',
        amount: item?.amount ?? item?.remaining_amount ?? item?.balance ?? item?.total ?? 0,
        days: item?.days ?? item?.daysOverdue ?? item?.overdueDays ?? item?.overdue_days ?? 0,
